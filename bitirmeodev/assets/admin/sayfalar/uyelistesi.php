...
$uyeID = $_SESSION['UyeID'];
  ...
  $uyeleriCek = $db->prepare("SELECT COUNT(*) as uyeSayisi FROM uye");
  $uyeleriCek->execute();
  $oyuncuSayisi = $uyeleriCek->fetch(PDO::FETCH_ASSOC)['uyeSayisi'];
  ...
  // Oyuncuları seç
  $uyeSorgusu = $db->prepare("SELECT* FROM uye  LIMIT :limit OFFSET :offset");
  $uyeSorgusu->bindParam(':limit', $limit, PDO::PARAM_INT);
  $uyeSorgusu->bindParam(':offset', $offset, PDO::PARAM_INT);
  $uyeSorgusu->execute();
  $uyeler = $uyeSorgusu->fetchAll(PDO::FETCH_ASSOC);
  

  if(isset($_GET['uyeID'])) {
    $uyeID = $_GET['uyeID'];
 
    $uyeSorgusu = $db->prepare("SELECT * FROM uye 
    WHERE ID = :uyeID");
    $uyeSorgusu->bindParam(':uyeID', $uyeID, PDO::PARAM_INT);
    $uyeSorgusu->execute();
    $uyeler = $uyeSorgusu->fetchAll(PDO::FETCH_ASSOC);
...
<nav class="navbar navbar-expand-sm navbar-dark bg-dark col-sm-12  ">
  ...
  <a href="../../../index.php" class="d-flex align-items-center mb-3 mb-md-0 me-md-auto text-white text-decoration-none">
    ...
    <ul class="navbar-nav me-auto">
      ...
      <form class="d-flex" action="" method="GET">
        <input class="form-control me-2" name="uyeID" type="text" placeholder="Üye ID girin">
      ...
      <button class="btn btn-danger m-2" onclick="yenile()">Sıfırla</button>
       <script>
          function yenile() {
            ...
            var cleanURL = url.replace(/[?&]uyeID=\d+/, ''); // uyeID parametresini temizler
    ...
    <ul class="nav nav-pills flex-column mb-auto">
      ...
      <li>
        <a href="uyelistesi.php" class="nav-link active">
  ...
  <div  class = "container">
                <h1 style="color: white; font-size: 60px; font-weight: bold;">Üye Listesi</h1>
                <table class="table">
                ...
                <input type="hidden" name="oyuncuID" value="<?php echo $uye['ID']; ?>">
                        <tr>
                            ...
                            <td><?php echo $uye['Ismi']; ?></td>
                            <td><?php echo $uye['Soyismi']; ?></td>
